...
delta1 =10;
delta2 = 0.25;
mu1 = [5*sqrt(2) 5*sqrt(2)];
mu2 = [-2.5*sqrt(2) 0];
mu3 = [5*sqrt(2) -5*sqrt(2)];
...
gnd_inst =[];
inst_data = [];
inst_data = [mvnrnd(mu1,sigma1,nSmp); mvnrnd(mu2,sigma1,2*nSmp); mvnrnd(mu3,sigma1,nSmp)]*R;
gnd_inst =  [ones(nSmp,1);-ones(2*nSmp,1);ones(nSmp,1)];

% inst_data = NormalizeFea(inst_data);
...
for parat = 1:length(t_set)
    for fold_id=1:Num_Kfolds
        ...
        primal_y_llda = (W_llda(2,1)*toy_x+c_llda)./W_llda(1,1);
